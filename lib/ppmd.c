    ...
    const void* src, size_t srcSize, int compressionLevel)
{
    ...
    return (cw.ptr <= cw.end) ? (cw.ptr - (char*)dst) : 0;
}
    ...
    const void* src, size_t compressedSize)
{
    ...
    unsigned b0, b1, wPPMd, modelOrder, memMb, restoreMethod, finishedOK;
    char *ptr, *ptrEnd;
    ISzAlloc ialloc = {pmalloc, pfree};
    if (compressedSize < 2)
        return 0;
    ...
    finishedOK = Ppmd8_RangeDec_IsFinishedOK(&ppmd) && cr.ptr == cr.end;
    Ppmd8_Free(&ppmd, &ialloc);
    return finishedOK ? (ptr - (char*)dst) : 0;
}
